...
GLuint gOffsetXLocation;
GLuint gOffsetYLocation;
GLuint gOffsetVec;
...
*/
	...
	scaleX += 0.03f;
	scaleY += 0.01f;
	glUniform1f(gOffsetXLocation, sinf(scaleX));
	glUniform1f(gOffsetYLocation, sinf(scaleY));*/
	vec2 pos(1,2);
	glUniform2fv(gOffsetVec,1, &pos[0]);
...
static void buildShaders()
{
	...
	gOffsetXLocation = glGetUniformLocation(shaderProgram, "gOffsetX");
	assert(gOffsetXLocation != 0xFFFFFFFF);
	
	gOffsetYLocation = glGetUniformLocation(shaderProgram, "gOffsetY");
	assert(gOffsetYLocation != 0xFFFFFFFF);*/
}
